...
impl Serializable for PartialMerkleTree {
   ...
   fn serialize(&self, io:&mut std::io::Write) -> serialize::Result {
      let mut r:usize = 0;
      r += try!(self.n_transactions.serialize(io));
      {
         let mut bytes = self.bits.to_bytes();
         for byte in &mut bytes {
            *byte = reverse_u8(*byte);
         }
         r += try!(bytes.serialize(io));
      }         
      r += try!(self.hashes.serialize(io));
      Ok(r)
   }
   fn unserialize(&mut self, io:&mut std::io::Read) -> serialize::Result {
      let mut r:usize = 0;
      r += try!(self.n_transactions.unserialize(io));
      {
         let mut bytes:Vec<u8> = Vec::new();
         r += try!(bytes.unserialize(io));
      ...
      r += try!(self.hashes.unserialize(io));
      Ok(r)
   }
}
